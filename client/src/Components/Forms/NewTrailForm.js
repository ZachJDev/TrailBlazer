...
export default function NewTrailForm({ handleSubmit, park, defaultValues }) {
  ...
  const [description, setTrailDescription] = useInputState(
    defaultValues?.description || ""
  );
  const [length, setTrailLength] = useInputState(defaultValues?.length || "");
  const [lengthUnit, setLengthUnit] = useState(
    defaultValues?.lengthUnit || "m"
  );
      ...
      <div>
        <InputGroup>
          ...
          <DropdownButton as={InputGroup.Append} title={lengthUnit}>
            <Dropdown.Item onSelect={handleDropdown} eventKey="m">
              Miles
            </Dropdown.Item>
            <Dropdown.Item onSelect={handleDropdown} eventKey="km">
              Kilometers
            </Dropdown.Item>
          </DropdownButton>
        </InputGroup>
