...
export default function UserComment({comment, depth, refreshCommentSection}) {
    ...
    const handleReply = async (replyText) => {
        const replyRequest = await sendReply({text:replyText,
    ...
    const handleDelete = async () => {
        const deleteRes = await sendDelete({commentId: comment.commentId, userId: comment.user.userId})
    ...
    if(isEditing) {
        commentComponent =  <CommentEditMode text={text} handleText={setText} submitOnClick={handleEdit} cancelOnClick={resetText}/>
    }
    else if(isReplying) {
        ...
        commentComponent =  <FlexWrapper direction={'column'}>
            ...
            <FlexWrapper className={"comment-actions"}>
                ...
                <span><FontAwesomeIcon className={"comment_edit-icon"} onClick={flipIsEditing} icon={faEdit}/> Edit</span>
            </FlexWrapper>
        </FlexWrapper>
    }
    return(
